...
*/
	printf("x264_sei_scalability_write down\n");
	...
	bs_write1(&q, b_temporal_id_nesting_flag);
	bs_write1(&q, b_priority_layer_info_present_flag);
	bs_write1(&q, b_priority_id_setting_flag);
	if(i_num_layers_minus1 + 1 <= 0 )
		return -1 ;
	bs_write_ue(&q, i_num_layers_minus1);
	...
	int b_layer_dependency_info_present_flag = 0;  // 
	int b_parameter_sets_info_present_flag = 1;
	int b_bitstream_restriction_info_present_flag = 0 ;
	int b_exact_interlayer_pred_flag = 0;
	int b_layer_conversion_flag = 0 ;
	int b_layer_output_flag = 1;
	
	for(int i = 0 ; i <= i_num_layers_minus1; i++)
		{
			bs_write_ue(&q, i );// layer_id
			bs_write(&q, 6, 0);//priority_id
			bs_write1(&q, 0); // discardable_flag
			bs_write(&q, 3, i);//dependency_id
			bs_write(&q, 4,0);//quality_id
			bs_write(&q, 3,0);//temporal_id
			
			bs_write1(&q, b_sub_pic_layer_flag);
			bs_write1(&q, b_sub_region_flag);
			bs_write1(&q, b_iroi_division_info_present_flag);
			bs_write1(&q, b_proflie_level_info_present_flag);
			bs_write1(&q, b_bitrate_info_present_flag);
			bs_write1(&q, b_frm_rate_info_present_flag);
			bs_write1(&q, b_frm_size_info_present_flag);
			bs_write1(&q, b_layer_dependency_info_present_flag);
			bs_write1(&q, b_parameter_sets_info_present_flag);
			bs_write1(&q, b_bitstream_restriction_info_present_flag);
			bs_write1(&q, b_exact_interlayer_pred_flag);

		
		        bs_write1(&q, b_layer_conversion_flag);
			bs_write1(&q, b_layer_output_flag);
			...
			if(b_parameter_sets_info_present_flag)
				{
					bs_write_ue(&q, 0);//"ScalableSEI: num_seq_parameter_set_minus1"		
					bs_write_ue(&q, 0);// "ScalableSEI: seq_parameter_set_id_delta"	
					bs_write_ue(&q, 0);//"ScalableSEI: num_subset_seq_parameter_set_minus1"
					bs_write_ue(&q, 0);//  "ScalableSEI: subset_seq_parameter_set_id_delta"	
					bs_write_ue(&q, 0);//"ScalableSEI: num_pic_parameter_set_minus1"
					bs_write_ue(&q, 0);//  "ScalableSEI: pic_parameter_set_id_delta"			
				}
		}
 x264_sei_write( s, tmp_buf, bs_pos( &q ) / 8, SEI_SCALABILITY_EXTESION);
	return 0;
