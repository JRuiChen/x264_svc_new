                                    ...
                                    int i_idr_pic_id, int i_frame, int i_qp )
{
    ...
    sh->b_constrained_intra_resampling_flag = 0;
  // 这个地方是不是还会扩展啊
...
static void x264_fdec_filter_row( x264_t *h, int mb_y, int pass )
{
	...
	printf("call function x264_fdec_filter_row HHHHHHHHHHHHHHHHH \n");
}
    ...
    while( 1 )
    {
        ...
        if( i_mb_x == 0 )
        {
            ...
            if( !h->mb.b_reencode_mb )
                x264_fdec_filter_row( h, i_mb_y, 0 );/*Point 1*/
        }
...
static void x264_slice_write_EL(x264_t* h)
{
  ...
  //x264_slice_header_write_EL( &h->out.bs, &h->sh, h->i_nal_ref_idc,&h->out.nal[h->out.i_nal] );
...
static void x264_write_all_slices(x264_t* h,int i_slice_num,int last_thread_mb,int b_base_layer)
{
...
static void *x264_slices_write( x264_t *h )
{
  ...
  // for(int i = 0;i < h->mb.i_mb_count;i++)
   //x264_log( NULL, X264_LOG_ERROR, "the mb type of current frame i_frame '%s' index '%s' type '%s' \n",h->i_frame, i, h->mb.type[i] );
/* 三个层共用sh，sh重新赋值*/
   ...
   /*printf test info - BY MING*/
   printf("HHHHHHHHHHHHHHHHHHHHHH  i_mb_count:%d \n",h->mb.i_mb_count);
